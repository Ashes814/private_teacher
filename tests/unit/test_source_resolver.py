...
class TestResolve:
    ...
    def test_resolve_page(self, resolver) -> None:
        loc = resolver.resolve(make_chunk(file_type="pdf", page=12, total_pages=50))
        assert loc.page == 12
...
class TestLabelAndUri:
    def test_label_with_lines(self, tmp_path, resolver) -> None:
        ...
        loc = resolver.resolve(
            make_chunk(source_path=str(f), source_name="a.py", line_start=10, line_end=25)
        )
    ...
    def test_label_single_line(self, tmp_path, resolver) -> None:
        ...
        loc = resolver.resolve(
            make_chunk(source_path=str(f), source_name="a.py", line_start=7, line_end=7)
        )
...
class TestReadSnippet:
    ...
    def test_context_clamped_at_boundaries(self, tmp_path, resolver) -> None:
        ...
        loc = resolver.resolve(make_chunk(source_path=str(f), line_start=1, line_end=1))
    ...
    def test_falls_back_for_pdf(self, resolver) -> None:
        ...
        loc = resolver.resolve(make_chunk("PDF 的内容", file_type="pdf", page=3, total_pages=10))
    ...
    def test_numbered_snippet_marks_hits(self, tmp_path, resolver) -> None:
        ...
        loc = resolver.resolve(make_chunk(source_path=str(f), line_start=5, line_end=6))
